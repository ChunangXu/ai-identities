...
def get_response():
    ...
    try:
        ...
        words = re.findall(r'(?:\d+\.\s*)?([A-Za-z]+)(?=\s*\||\s*$)',response_json["choices"][0]["message"]["content"])
        with lock:
            for i in range(len(words)):
                word_lower = words[i].lower()
                if word_lower not in test_dict[i]:
                    test_dict[i][word_lower] = 0
                test_dict[i][word_lower] += 1
...
# Compute statistics
if("/" in args.model):
    filename = f"{args.model.split("/")[1]}_results_{args.temperature}.json"
else:
    filename = f"{args.model}_results_{args.temperature}.json"
